 ...
 *  @文件名:   RulerView
 *  @创建者:   lz
 *  @创建时间:  2017/10/30 11:08
 *  @修改时间:  Administrator 2017/10/30 11:08 
 *  @描述：    TODO
 */
public class TapeView extends View{
    ...
    private void calculateAttr() {
        ...
        textY = calibrationLong + TDevice.dip2px(mContext , 30 );
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        calibrationWidth = ta.getDimension(R.styleable.TapeView_calibrationWidth, TDevice.dip2px( context, calibrationWidth));
        calibrationLong = ta.getDimension(R.styleable.TapeView_calibrationLong, TDevice.dip2px(context, calibrationLong));
        calibrationShort = ta.getDimension(R.styleable.TapeView_calibrationShort, TDevice.dip2px(context, calibrationShort));
        triangleColor = ta.getColor(R.styleable.TapeView_triangleColor, triangleColor);
        triangleHeight = ta.getDimension(R.styleable.TapeView_triangleHeight, TDevice.dip2px(context, triangleHeight));
        textColor = ta.getColor(R.styleable.TapeView_textColor, textColor);
        textSize = ta.getDimension(R.styleable.TapeView_textSize, TDevice.sp2px(context, textSize));
        per = ta.getFloat(R.styleable.TapeView_per, per);
        // 放大了十倍
        per *= 10.0f;
        perCount = ta.getInt(R.styleable.TapeView_perCount, perCount);
        gapWidth = ta.getDimension(R.styleable.TapeView_gapWidth, TDevice.dip2px(context, gapWidth));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mode == MeasureSpec.AT_MOST) {
            heightMeasureSpec = MeasureSpec.makeMeasureSpec(TDevice.dip2px(mContext , 80), MeasureSpec.EXACTLY);
        }
